...
public class Cache<T: DataConvertible where T.Result == T, T : DataRepresentable> {
    ...
    public init(name : String) {
        ...
        addFormat(originalFormat)
    }
    
    deinit {
        if let memoryWarningObserver = memoryWarningObserver {
            let notificationCenter = NSNotificationCenter.defaultCenter()
            notificationCenter.removeObserver(memoryWarningObserver, name: UIApplicationDidReceiveMemoryWarningNotification, object: nil)
        }
    }
    ...
    private func setupMemoryObserver() {
        ...
        let notificationCenter = NSNotificationCenter.defaultCenter()
        memoryWarningObserver = notificationCenter.addObserverForName(
            UIApplicationDidReceiveMemoryWarningNotification,
            object: nil,
            queue: NSOperationQueue.mainQueue(),
            usingBlock: { [unowned self] (notification : NSNotification!) in
    ...
    public func set(#value : T, key: String, formatName : String = HanekeGlobals.Cache.OriginalFormatName, success succeed : ((T) -> ())? = nil) {
        if let (format, memoryCache, diskCache) = self.formats[formatName] {
            self.format(value: value, format: format) { formattedValue in
                ...
                diskCache.set(key, value: self.dataFromValue(formattedValue, format: format))
    ...
    public func fetch(#key : String, formatName : String = HanekeGlobals.Cache.OriginalFormatName, failure fail : Fetch<T>.Failure? = nil, success succeed : Fetch<T>.Success? = nil) -> Fetch<T> {
        ...
        if let (format, memoryCache, diskCache) = self.formats[formatName] {
            if let wrapper = memoryCache.objectForKey(key) as? ObjectWrapper {
                if let result = wrapper.value as? T {
                    ...
                    diskCache.updateAccessDate(key, value: self.dataFromValue(result, format: format))
    ...
    public func fetch(#fetcher : Fetcher<T>, formatName : String = HanekeGlobals.Cache.OriginalFormatName, failure fail : Fetch<T>.Failure? = nil, success succeed : Fetch<T>.Success? = nil) -> Fetch<T> {
        let key = fetcher.key
        let fetch = Cache.buildFetch(failure: fail, success: succeed)
    ...
    private class func buildFetch(failure fail : Fetch<T>.Failure? = nil, success succeed : Fetch<T>.Success? = nil) -> Fetch<T> {
        let fetch = Fetch<T>()
    ...
    public func fetch(#key : String, @autoclosure(escaping) value getValue : () -> T.Result, formatName : String = HanekeGlobals.Cache.OriginalFormatName, success succeed : Fetch<T>.Success? = nil) -> Fetch<T> {
    ...
    public func fetch(#path : String, formatName : String = HanekeGlobals.Cache.OriginalFormatName,  failure fail : Fetch<T>.Failure? = nil, success succeed : Fetch<T>.Success? = nil) -> Fetch<T> {
        let fetcher = DiskFetcher<T>(path: path)
        return self.fetch(fetcher: fetcher, formatName: formatName, failure: fail, success: succeed)
    }
    
    public func fetch(#URL : NSURL, formatName : String = HanekeGlobals.Cache.OriginalFormatName,  failure fail : Fetch<T>.Failure? = nil, success succeed : Fetch<T>.Success? = nil) -> Fetch<T> {
